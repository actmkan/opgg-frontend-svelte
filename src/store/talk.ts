...
export const onLike = (target, targetId, type) => {
    toggleLike(target, targetId, type)
        .then(() => {
            const articleInfo: any = get(article);
            if (target === "ARTICLE") {
                fetchArticle(articleInfo.talk_id, articleInfo.channel_id, targetId);
            }
            if (target === "COMMENT") {
                const commentSortInfo: any = get(commentSort);
                const commentsInfo: any = get(comments);
                getComments(
                    articleInfo.talk_id,
                    articleInfo.channel_id,
                    articleInfo.id,
                    commentSortInfo,
                    1,
                    commentsInfo.length
                ).then((r) => {
                    comments.set([...r.data.data]);
                });
            }
        })
        .catch((e) => {
...
export const setCommentSort = (sortName) => {
    const articleInfo: any = get(article);
    commentSort.set(sortName);
    fetchComments(articleInfo.talk_id, articleInfo.channel_id, articleInfo.id, 1);
};
